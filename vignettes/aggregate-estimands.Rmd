...
- **avg_of_ratios** ($\theta_{\text{Agg},1}$): weighted average of each group's normalized effect $\theta(g,d,d+e)$. Preferred because normalization happens within each group before aggregation, so baseline differences do not inflate the average.
- **ratio_of_avgs** ($\theta_{\text{Agg},2}$): ratio of the weighted-average ATE to the weighted-average APO. This implicitly up-weights high-earning groups (because a given ATE is a smaller share of a larger APO), so it differs from $\theta_{\text{Agg},1}$ when earnings vary across groups.
- **gender_ineq** ($\Delta\rho_{\text{Agg}}$): weighted average of `NTD_Alt` theta estimates — the aggregate gender-inequality penalty.
...
Plot the `avg_of_ratios` aggregate for the NTD method across event times, with 95% confidence ribbons.
